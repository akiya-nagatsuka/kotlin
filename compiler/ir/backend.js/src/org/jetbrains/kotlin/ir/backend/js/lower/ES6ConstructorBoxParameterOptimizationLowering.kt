...
class ES6ConstructorBoxParameterOptimizationLowering(private val context: JsIrBackendContext) : FileLoweringPass {
    ...
    private fun IrSimpleFunction.getOrCreateReplacementWithoutBoxParameter(): IrSimpleFunction? {
        ...
        if (!isEs6ConstructorReplacement || parentAsClass.needsBoxParameter || valueParameters.none { it.isBoxParameter }) return null
        ...
        return factory.buildFun {
            ...
            original.replacementWithoutBoxParameter = this
            body = original.body?.transform(
                object : IrElementTransformerVoid() {
                    ...
                    override fun visitCall(expression: IrCall): IrExpression {
                        return if (expression.isSuperCallWithBoxParameter) {
                            expression.putValueArgument(expression.valueArgumentsCount - 1, context.getVoid())
                            super.visitCall(expression)
                        } else {
                            super.visitCall(expression.replaceCalleeIfNeeded())
                        }
                    }
                },
                null
